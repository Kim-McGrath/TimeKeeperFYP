...
class MetronomeEngine {
     ...
     * @param coroutineScope Coroutine scope for the metronome loop
     * @return The timestamp when the metronome rhythm starts (beat 0)
     */
    ...
    private fun generateClickSound(): ShortArray {
        ...
        for (i in 0 until samples) {
            ...
            // Combine and scale to 16-bit PCM range
            val sample = (sine * envelope * 0.3 * Short.MAX_VALUE).toInt()
    ...
    /**
     * Updates the BPM while playing.
     * Requires stopping and restarting for smooth transition.
     */
    fun setBpm(newBpm: Int, coroutineScope: CoroutineScope) {
        if (isPlaying) {
            stop()
            start(newBpm, coroutineScope)
        } else {
            bpm = newBpm
        }
    }
